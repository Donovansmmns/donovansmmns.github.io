...
function setup() {
  ...
  ballArray.push(new Ball(width/2, height/2, random(-15, 15), random(-15, 15), 50));
}
...
function draw() {
  ...
  for (let i = 0; i < ballArray.length; i++){
    ...
    for (let j = 0; j < ballArray.length; j++){
      ...
      // else{
      //   ballArray[i].fillColor = color(255);
      //   ballArray[j].fillColor = color(255);
      // }
    }
...
function keyPressed(){
  if (key === " "){
    ballArray.push(new Ball(mouseX, mouseY, random(-15, 15), random(-15, 15), 50));
